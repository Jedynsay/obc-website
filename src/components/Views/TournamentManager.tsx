...
import {
  Plus, Edit, Trash2, Save, X, Calendar, MapPin, Users, Trophy, Eye, EyeOff
} from 'lucide-react';
...
export function TournamentManager() {
  ...
  const deleteTournament = async (id: string) => {
    ...
    try {
      // Your original cascade deletes were kept conceptually; adapt to your DB rules as needed.
      await supabase.from('tournament_beyblade_parts').delete().eq('beyblade_id',
        `(SELECT id FROM tournament_beyblades WHERE registration_id IN (SELECT id FROM tournament_registrations WHERE tournament_id = '${id}'))` as any
      );
      await supabase.from('tournament_beyblades').delete().eq('registration_id',
        `(SELECT id FROM tournament_registrations WHERE tournament_id = '${id}')` as any
      );
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div
          key={r.id}
          className="border border-slate-700 bg-slate-900/50 p-4"
        >
