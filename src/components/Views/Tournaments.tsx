  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {/* <div className="bg-white rounded-x1 shadow-sm border border-gray-200 p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              <input
                ...
                className="w-full border border-gray-300 rounded-x1 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono"
                maxLength={8}
              />
            </div>
            <button
              onClick={searchTournamentByCode}
              className="bg-blue-600 text-white px-6 py-2 rounded-x1 hover:bg-blue-700 transition-colors"
            >
              Search Tournament
            </button>
          </div>
          ...
          {codeSearchResult && (
            <div className="mt-4">
              {codeSearchResult.error ? (
                <div className="bg-red-50 border border-red-200 rounded-x1 p-4">
                  <p className="text-red-700">{codeSearchResult.error}</p>
                </div>
              ) : (
                <div className="bg-green-50 border border-green-200 rounded-x1 p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      {codeSearchResult.is_practice && (
                        <span className="inline-block bg-purple-100 text-purple-800 px-2 py-1 rounded-x1 text-xs font-medium mt-2">
                          Practice Tournament
                        </span>
                      )}
                    </div>
                    <button
                      ...
                      className="bg-green-600 text-white px-4 py-2 rounded-x1 hover:bg-green-700 transition-colors disabled:opacity-50"
                    >
 ...
 */}
        {/* Filter Tabs */}
        <div className="bg-slate-900/50 border border-cyan-500/30 rounded-x1 p-6 backdrop-blur-sm mb-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
            <div className="flex items-center space-x-1 bg-slate-800/50 border border-cyan-500/20 rounded-x1 p-1">
        ...
        {loading ? (
          <div className="text-center py-12">
            <div className="w-12 h-12 border-4 border-cyan-500/30 border-t-cyan-500 rounded-x1 animate-spin mx-auto mb-4"></div>
            <p className="text-slate-400">Loading tournaments...</p>
          </div>
        ) : filteredTournaments.length === 0 ? (
          <div className="text-center py-12">
            <div className="w-16 h-16 bg-slate-800/50 rounded-x1 flex items-center justify-center mx-auto mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTournaments.map((tournament) => (
              <div key={tournament.id} className="bg-slate-900/50 border border-cyan-500/30 rounded-x1 p-6 backdrop-blur-sm hover:shadow-[0_0_30px_rgba(0,200,255,0.2)] transition-all duration-300 group">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className={`px-3 py-1 rounded-x1 text-xs font-medium capitalize ${
                      ...
                      {tournament.is_practice && (
                        <span className="px-3 py-1 rounded-x1 text-xs font-medium bg-purple-500/20 text-purple-400 border border-purple-500/30">
                          Practice
                        </span>
                      )}
                    </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="w-full bg-slate-800 rounded-x1 h-2">
                    <div 
                      className="h-2 rounded-x1 bg-gradient-to-r from-cyan-500 to-purple-500 transition-all duration-500"
                      style={{ 
                ...
                {tournament.status === 'upcoming' && (
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-medium px-6 py-3 rounded-x1 hover:from-cyan-400 hover:to-purple-500 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_20px_rgba(0,200,255,0.3)] group-hover:shadow-[0_0_30px_rgba(0,200,255,0.5)]"
                  >
