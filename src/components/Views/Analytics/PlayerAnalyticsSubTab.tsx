        ...
        <div className="p-6 bg-gray-800">
          <div className="mb-4 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 bg-gray-700 text-white"
            />
          </div>
          <div className="overflow-auto max-h-[60vh]">
            <table className="min-w-full divide-y divide-gray-700">
              <thead className="bg-gray-700 sticky top-0">
                <tr>
                  {columns.map(col => (
                    <th key={col.key} className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase">{col.label}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="bg-gray-800 divide-y divide-gray-700">
                {filteredData.map((row, index) => (
                  <tr key={index} className={`hover:bg-gray-700 ${onRowClick ? 'cursor-pointer' : ''}`} onClick={() => onRowClick?.(row)}>
                    {columns.map(col => (
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-gray-800 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden border border-gray-700">
        <div className="bg-gradient-to-r from-green-600 to-blue-600 px-6 py-4 text-white flex justify-between items-center">
          <h2 className="text-2xl font-bold">{title}</h2>
          <button onClick={onClose} className="p-2 hover:bg-white/20 rounded-full">
            <X size={24} />
          </button>
        ...
        <div className="p-6 overflow-auto max-h-[60vh] bg-gray-800">
          <table className="min-w-full divide-y divide-gray-700">
            <thead className="bg-gray-700">
              <tr>
                <th className="px-6 py-3 text-xs font-medium text-gray-300 uppercase">Result</th>
                <th className="px-6 py-3 text-xs font-medium text-gray-300 uppercase">Player 1</th>
                <th className="px-6 py-3 text-xs font-medium text-gray-300 uppercase">Player 1 Beyblade</th>
                <th className="px-6 py-3 text-xs font-medium text-gray-300 uppercase">Player 2</th>
                <th className="px-6 py-3 text-xs font-medium text-gray-300 uppercase">Player 2 Beyblade</th>
                <th className="px-6 py-3 text-xs font-medium text-gray-300 uppercase">Winner</th>
                <th className="px-6 py-3 text-xs font-medium text-gray-300 uppercase">Finish Type</th>
              </tr>
            </thead>
            <tbody className="bg-gray-800 divide-y divide-gray-700">
              {matches.map((match, index) => (
                ...
                <tr key={index} className={`hover:bg-gray-700 ${
                  match.winner_name === match.player1_name ? 'bg-blue-900/20' :
                  match.winner_name === match.player2_name ? 'bg-red-900/20' : ''
                }`}>
                  <td className="px-6 py-4 text-sm font-medium text-white">Match {index + 1}</td>
                  <td className={`px-6 py-4 text-sm font-medium ${
                    match.winner_name === match.player1_name ? 'text-blue-400' : 'text-gray-400'
                  }`}>{match.player1_name}</td>
                  <td className="px-6 py-4 text-sm font-mono text-xs text-gray-300">{match.player1_beyblade}</td>
                  <td className={`px-6 py-4 text-sm font-medium ${
                    match.winner_name === match.player2_name ? 'text-red-400' : 'text-gray-400'
                  }`}>{match.player2_name}</td>
                  <td className="px-6 py-4 text-sm font-mono text-xs text-gray-300">{match.player2_beyblade}</td>
                  <td className="px-6 py-4 text-sm font-medium text-green-400">{match.winner_name}</td>
                  <td className="px-6 py-4 text-sm text-gray-300">{match.outcome}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 p-6 mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-700">
                <thead className="bg-gray-700">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-600">Player</th>
                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-600">Matches</th>
                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-600">Win Rate</th>
                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-600">Weighted Win Rate</th>
                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-600">Total Points</th>
                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-600">Avg Points/Match</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-600">Most Common Win</th>
                  </tr>
                </thead>
                <tbody className="bg-gray-800 divide-y divide-gray-700">
                  {Object.values(players)
                    ...
                    .map((player, index) => (
                      <tr
                        key={player.name}
                        className={`hover:bg-gray-700 cursor-pointer ${selectedPlayer === player.name ? 'bg-blue-900/30' : ''}`}
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-sm text-center text-gray-300">{player.matches}</td>
                        ...
                        <td className="px-6 py-4 text-sm font-bold text-blue-400 text-center">
                          {(player.weightedWinRate * 100).toFixed(1)}%
                        </td>
                        <td className="px-6 py-4 text-sm font-medium text-center text-gray-300">{player.totalPoints}</td>
                        <td className="px-6 py-4 text-sm text-center text-gray-300">{player.avgPointsPerMatch.toFixed(2)}</td>
                        <td className="px-6 py-4 text-sm text-gray-300">{player.mostCommonWinFinish}</td>
                      </tr>
                    ))}
                </tbody>
              </table>
        ...
        <div className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-400">{selectedPlayerData.matches}</div>
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-gray-400">Most Valuable Beyblade</div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-700">
                <thead className="bg-green-900/20">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Beyblade</th>
                    {FINISH_TYPES.map(finish => (
                      <th key={finish} className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">
                        {finish.split(' ')[0]}
                      </th>
                    ))}
                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Total Points Gained</th>
                  </tr>
                </thead>
                <tbody className="bg-gray-800 divide-y divide-gray-700">
                  {Object.keys(selectedPlayerData.winsByFinish).map(beyblade => (
                    <tr key={beyblade} className="hover:bg-gray-700">
              ...
              <table className="min-w-full divide-y divide-gray-700">
                <thead className="bg-red-900/20">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Total Points Given</th>
                  </tr>
                </thead>
                <tbody className="bg-gray-800 divide-y divide-gray-700">
          ...
          <div className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 p-6">
            ...
            <ResponsiveContainer width="100%" height={400}>
              ...
              >
                ...
                <PolarAngleAxis dataKey="finish" tick={{ fill: '#9CA3AF', fontSize: 12 }} />
        ...
        <div className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 p-6">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-700">
              <thead className="bg-gray-700">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Matchup</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Total Matches</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Player 1 Wins</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Player 2 Wins</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Player 1 Win Rate</th>
                </tr>
              </thead>
              <tbody className="bg-gray-800 divide-y divide-gray-700">
                {headToHead
                  ...
                  .map((h2h, index) => (
                    <tr key={index} className="hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-300 text-center">{h2h.totalMatches}</td>
