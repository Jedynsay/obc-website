...
export function QuickAccessCards({ stats, upcomingTournaments, deckPresets, onViewChange }) {
  return (
    <section className="max-w-7xl mx-auto px-6 py-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
...
function Card({ onClick, icon, title, children, accent }) {
  ...
  return (
    <button
      onClick={onClick}
      className="bg-slate-900 border border-slate-800 rounded-lg p-6 text-left hover:border-blue-500 transition-all"
    >
      <div className={`w-10 h-10 ${accentColors[accent]} rounded-lg flex items-center justify-center mb-4`}>
        {icon}
      </div>
